...
Build: `.github/workflows/mobile-apk.yml` — tag `vX.Y.Z` → signed APK on the GitHub Release (`solray-X.Y.Z.apk`). Flutter pinned to 3.24.3 in CI (matches local verification). Signing: secrets `ANDROID_KEYSTORE_BASE64/PASSWORD/KEY_ALIAS/KEY_PASSWORD`, or a generated build key when absent.
